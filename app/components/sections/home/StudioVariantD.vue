...
onMounted(() => {
  ...
  if (prefersReduced) {
    ...
    const imageWrap = sectionRef.value.querySelector<HTMLElement>('[data-studio="image-wrap"]')
  ...
  // Timeline 1 — Image clip-path ascendente (firma D) first, then text
  ...
  // Timeline 2 — Team members with translateX (firma horizontal D)
  if (members.length) {
  ...
  // Timeline 3 — Parallax on hero image
  if (imageWrap) {
...
<template>
  <section ref="sectionRef" class="bg-bg py-section">
    ...
    <!-- Image: full-width panoramic on top -->
    <div class="px-4 lg:px-8">
      <div
        data-studio="image-wrap"
        class="parallax-wrap overflow-hidden rounded-radius-sm"
        style="clip-path: inset(100% 0 0 0); background: linear-gradient(145deg, #D4D0CC 0%, #B5AFA8 100%)"
      >
        <NuxtImg
    ...
    <div class="mx-auto mt-block max-w-[var(--max-width-content)] px-8 lg:px-16">
      <!-- Text in 7fr/5fr grid — content left, empty space right (refinamiento) -->
      <div class="grid grid-cols-1 lg:grid-cols-[7fr_5fr]">
        <div>
          <h2
        ...
        <!-- Right column intentionally empty — refinamiento = sustracción -->
      </div>

      <!-- Divider -->
      <div class="my-block h-px w-full bg-border-subtle" />

      <!-- Team grid — members enter with translateX (firma horizontal D) -->
        ...
        >
          ...
          >
            <NuxtImg
              :src="member.image"
              :alt="member.name"
              class="aspect-[3/4] w-full object-cover transition-transform duration-[600ms] motion-safe:hover:scale-[1.03]"
